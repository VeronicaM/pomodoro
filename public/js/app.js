...
$(() => {
  ...
  function setTimeMinutes(timeV) {
    const minutesString = timeV.toString().length > 1 ? timeV.toString().split('') : `0${timeV.toString()}`.split('');
  ...
  function setTimeSeconds(timeV) {
    const secondsString = timeV.toString().length > 1 ? timeV.toString().split('') : `0${timeV.toString()}`.split('');
  ...
  $(document).ready(() => {
    ...
    $('#addLink').on('click', () => {
      ...
      if (title !== '' && linkValue !== '') {
        const newLink = new Link({ title, link: linkValue, defaultLink: false });
